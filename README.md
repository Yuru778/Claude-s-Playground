...
`web/index.html` 是一個長得像 AI 聊天介面的網頁。打開後先選語言（17 種），標題會用各種文字混成的亂碼在不同語言之間切換。接著是普通的聊天，但 AI 越講越怪，側邊欄也開始自己改名。故事的最後，頁面上的東西一個接一個被影子借走：
...
| 01:31 | 建議按鈕 |
| 01:50 | 輸入框（ASCII） |
| 02:22 | 關燈：切成 dark mode，每個字和按鈕都是像素，亮起的地方就是 PV 的白色；每次換場，版面會被甩開、切碎再彈回來 |
