...
`web/index.html` 是一個長得像 AI 聊天介面的網頁。打開就是普通的聊天（依瀏覽器語言自動切換，支援 17 種語言），但 AI 越講越怪，側邊欄也開始自己改名。故事的最後，頁面上的東西一個接一個被影子借走：
...
按下「開始」後會先用遮罩蓋住頁面、箭頭指向播放器，讓你把 YouTube 的廣告略過（如果有的話），準備好再開始。
...
- 網址加上 `?lang=ja` 指定語言，`#play` 跳過故事，`#t=142` 從第 142 秒開始
